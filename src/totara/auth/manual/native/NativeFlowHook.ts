...
import type { NetworkError } from '@totara/types/Error';
import { useEffect, useReducer } from 'react';

import type { ManualFlowChildProps } from '../ManualFlowChildProps';
...
export const useNativeFlow =
  ...
  ({ siteUrl }: ManualFlowChildProps) => {
    ...
    const loginPromise = loginSecret =>
      fetchData<SetupSecret>(config.nativeLoginUri(siteUrl), {
        method: 'POST',
        body: JSON.stringify({
    ...
    useEffect(() => {
      if (nativeLoginState.isRequestingLogin && !nativeLoginState.setupSecret) {
        loginSecretPromise()
          .then(s => loginPromise(s.loginsecret))
          .then(setupSecret => {
